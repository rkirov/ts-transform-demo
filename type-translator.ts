...
// from
// https://raw.githubusercontent.com/angular/tsickle/master/src/type-translator.ts
...
/**
 * @return True if the named type is considered compatible with the
 * Closure-defined
 *     type of the same name, e.g. "Array".  Note that we don't actually enforce
 *     that the types are actually compatible, but mostly just hope that they
 * are due
...
export function typeToDebugString(type: ts.Type): string {
  ...
  const basicTypes: ts.TypeFlags[] = [
    ...
    ts.TypeFlags.EnumLiteral,   ts.TypeFlags.ESSymbol,
...
export class TypeTranslator {
  /**
   * A list of types we've encountered while emitting; used to avoid getting
   * stuck in recursive
   * types.
   */
  ...
  public symbolToString(sym: ts.Symbol): string {
    ...
    let writer: ts.SymbolWriter = {
      ...
      clear : doNothing,
      trackSymbol(symbol: ts.Symbol, enclosingDeclaration?: ts.Node,
                  meaning?: ts.SymbolFlags) { return; },
  ...
  translate(type: ts.Type): string {
    ...
    // in this switch statement, and should be kept in sync with
    // typescript.d.ts.
  ...
  private translateTypeLiteral(type: ts.Type): string {
    ...
    if (ctors.length) {
      // TODO(martinprobst): this does not support additional properties defined
      // on constructors
    ...
    for (let field of Object.keys(type.symbol.members)) {
      ...
      default:
        ...
        // optional members are handled by the type including |undefined in a
        // union type.
